...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(vscode.commands.registerTextEditorCommand('higgycursor.insertCursorBelow', insertCursorBelow));
  context.subscriptions.push(vscode.commands.registerTextEditorCommand('higgycursor.insertCursorAbove', insertCursorAbove));
  context.subscriptions.push(vscode.commands.registerTextEditorCommand('higgycursor.lastCursorLeft',    lastCursorLeft));
  context.subscriptions.push(vscode.commands.registerTextEditorCommand('higgycursor.lastCursorRight',   lastCursorRight));
}
...
function buildOptionRegExp(triggers: string) {
  return triggers.split('')
    .map((s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')) // $& means the whole matched string
    .reduce((a, b) => a + '|' + b);
}
